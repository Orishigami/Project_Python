...
def draw_roi(event, x, y, flags, param):
    global roi_start, roi_end, rois
    cam_id, cam_pos = param  # Get camera ID and position in grid
    if event == cv2.EVENT_LBUTTONDOWN:
        roi_start = (x - cam_pos[0], y - cam_pos[1])
        roi_end = None
    elif event == cv2.EVENT_MOUSEMOVE and roi_start:
        roi_end = (x - cam_pos[0], y - cam_pos[1])
    ...
    while True:
        for i, camera in enumerate(cameras):
            ...
            if ret:
                ...
                if selected_cameras[i] in rois:
                    for start, end in rois[selected_cameras[i]]:
                        cv2.rectangle(
                            grid[cam_y:cam_y+240, cam_x:cam_x+426], start, end, (0, 255, 0), 2
                        )

                cv2.setMouseCallback("Multi Camera View", draw_roi, (selected_cameras[i], (cam_x, cam_y)))

        if roi_start and roi_end:
            cam_idx = selected_cameras[0]
            start_pt = (roi_start[0] % 426, roi_start[1] % 240)
            end_pt = (roi_end[0] % 426, roi_end[1] % 240)
